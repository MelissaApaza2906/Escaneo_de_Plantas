...
async function fetchWikiThumbnailsBatch(names: string[]): Promise<Map<string, string>> {
  ...
  // redirects=1 matters a lot here: many scientific names are thin stub
  // articles that redirect to a richer common-name article with the real
  // photo (e.g. "Solanum tuberosum" -> "Potato", "Salvia rosmarinus" ->
  // "Rosemary" via "Rosmarinus officinalis"). Without it MediaWiki returns
  // the stub as-is, with no thumbnail, even though a photo exists one hop away.
  const url = `https://en.wikipedia.org/w/api.php?action=query&titles=${encodeURIComponent(names.join('|'))}&prop=pageimages&format=json&pithumbsize=600&redirects=1&origin=*`;
  try {
