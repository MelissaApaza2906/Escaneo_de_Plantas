...
- Plant catalog photos (all 118 entries — see `plants` in `App.tsx`) have no local image files; they're 100% external URLs. Previously **every** entry's `photo` field was force-set to `https://source.unsplash.com/600x400/?<name>+bolivia` (Unsplash's old "Source" endpoint) — confirmed dead, returns `503` for every request now. A `useEffect` then tried to replace each one with a real photo from Wikipedia's `pageimages` API, `onError` on the `<img>` swapping to a Pexels stock photo if a URL ever failed to load at runtime. **The actual bug behind "some gallery images don't load":** that effect fired one `fetch` per plant, all 118 in parallel via `Promise.all` with no throttling, which reliably trips Wikipedia's API rate limit — reproduced live: only ~10% of 58 simultaneous single-title requests succeeded, the rest got a plain-text "too many requests" body that fails `response.json()` and silently falls back to a generic placeholder SVG. Not a 404/CORS error, and not literally "broken" — visually inconsistent (some species got a real photo, most got a generic gray icon), which is what "no cargan correctamente" actually meant. Fixed by: (1) deleting the dead Unsplash step entirely — `photo` now starts directly at the placeholder SVG instead of a URL guaranteed to 503; (2) batching the Wikipedia lookups via MediaWiki's `titles=A|B|C` (pipe-separated, up to 50 titles per request) instead of one request per plant — 118 requests become ~3, verified live to return correct per-title results including the `normalized` title mapping. `onError` → Pexels fallback is kept as a third layer for genuine runtime failures (e.g. a Wikipedia thumbnail URL that stops resolving after being cached).
